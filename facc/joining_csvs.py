...
inpath= "/work/malla/meteo_germany/DE_indicatorsdata/facc/dams_output_csv/"
outpath="/work/malla/meteo_germany/DE_indicatorsdata/facc/final_dam_csvs/"

dams= pd.read_csv("/work/malla/meteo_germany/DE_indicatorsdata/facc/Dams_for_qrouted1_new.csv", encoding = "ISO-8859-1") # error_bad_lines=False)

for type in types:
    ind="Qrouted_"+type+"_year_"
    print(ind)
    for v in vars:
        list=[]
        s_file=pd.read_csv(inpath+"Qrouted_sum_year_abs-rel_change_rcp26_ensmaximum_germany_facc1.csv", encoding="ISO-8859-1") #, error_bad_lines=False)
        ...
        data.to_csv(outpath+ind+v+"_change_ensstats_updated_forlatex.csv", sep=";", encoding = "ISO-8859-1") #seprator change when converting all points to commas
        print(data)  
