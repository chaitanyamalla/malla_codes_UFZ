...
indicator_longnames_de={'n_heatdays'             : 'mittleren Anzahl Hitzetage',
                        ...
                        'Qrouted_sum'             : 'mittlere Abflusssumme',
...
if __name__ == '__main__':
    ...
    inpath="/work/malla/meteo_germany/DE_indicatorsdata/facc/final_dam_csvs_corrected/decimal_corrected/daily/"
    outpath="/work/malla/meteo_germany/DE_indicatorsdata/facc/final_dam_csvs_corrected/decimal_corrected/daily/"
    
    

    if eval_stat == "ensemble":
        table_header=colorbarlabel+ " sowie statistische Kennzahlen (Minimum, 25. Perzentil, Median, arithmet. Mittel, 75. Perzentil, Maximum)' über die Teilensembles unter RCP 2.6 (21 Klima-Hydrologie-Simulationen) und RCP 8.5 (49 Klima-Hydrologie-Simulationen) gezeigt."

        types=["forlatex", "forlatex_greater50facc", "forlatex_less50facc"]

        
        for t in types:
            latex_label=  indicator + "_Talsperren"+ t
            file=inpath+"Qrouted_daily_sum_year_abs-rel_change_ensstats_updated_"+t+".csv"
            print(file)
        
            texfile=outpath + indicator + '_' + eval_period + '_' + var_type+"_" +eval_stat+'_stat_'+t+'.tex'


            th = open(texfile, 'w')
            columns=["min","p25","median","p75","max","min","p25","median","p75","max"]
            # --------------------------------------------------
            #  -- WRITING TABLE --------------------------------
            # --------------------------------------------------
            ...
            th.write('\scriptsize')
            th.write('\n')
            th.write('\\begin{longtable}{@{\extracolsep{\\fill}}lc|ccccc||cccccc}')
            th.write('\n')
