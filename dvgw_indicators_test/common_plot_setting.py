...
if (pdffile == ''):
    ...
    outtype = 'pdf'
indicator_options =['pre_sum', 'n_heatdays','n_summerdays', 'n_consec_heatdays',
...
var_type_options =['rel_change', 'abs_change', 'abs', 'abs-abs_change', 'abs-rel_change','SNR']
...
ens_stat_options       = ['ensmedian', 'ensminimum','ensp90','ensmaximum','worstcase','SNR']
...
# if var_type == 'abs':
colorbarlabel_abs  = indicator_name +' ' + eval_period_name  + ',' + var_type_name['abs'] 
if indicator == "n_heatdays":
...
if indicator in ["pre_gt0p1_R90p","pre_gt0p1_R95p","pre_gt0p1_R99p","pre_gt1_R90p","pre_gt1_R95p","pre_gt1_R99p"]:
    colors_abs              = colors_PuBu9
    levels_abs            = np.arange(0,50,5)
colors_SNR = colors_Blues9 + whitewhite + colors_Reds9
levels_SNR = np.arange(-10,10,1)
colors_SNR =["#54278f","#807dba","#fe9929","#cc4c02","#662506"]
levels_SNR = np.arange(0,2.6,0.5)
colorbarlabel_SNR  = indicator_name +' ' + eval_period_name  + ' ' + "Signal to Noise Ratio" 
# create colormap
if var_type == 'abs':
