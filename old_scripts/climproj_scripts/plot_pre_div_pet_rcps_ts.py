...
# --------------------------------------------------
#  File:       plot_pre_div_pet_rcps_ts.py
#
...
indicator2= "pre"
pre_file = pd.read_csv(inpath+ indicator2 +"_allMETfiles.csv")

pet_file = pet_file.set_index('year')
pre_file = pre_file.set_index('year')


data = pre_file.div(pet_file)
...
plt.text(x=0.5, y=0.91, s='(Precipitation / Potential Evapotranspiration) yearly timeseries on 88 Models', fontfamily="Comic Sans MS", fontsize=12, ha="center", transform=fig.transFigure)

plt.show()

#fig.savefig(inpath+"pre_div_pet_rollingmean30yrs_RCP_ts.png")
#fig.savefig(inpath+"pet_div_pre_RCP_ts.png")
