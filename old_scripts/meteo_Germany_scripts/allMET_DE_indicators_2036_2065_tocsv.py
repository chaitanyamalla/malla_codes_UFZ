...
# --------------------------------------------------
#  File:        allMET_DE_indicators_2036_2065_tocsv.py 
#
#  Created:     Mi 28-07-2021
#  Author:      Chaitanya Malla
#  
# --------------------------------------------------
#
#  Description: script to produce csv of all METfiles(RCP26, 85) with different indicators for region GERMANY for year 2036_2065
#
#  Modified by: 
#  Modified date:
#
...
for met in mets:
    ...
    timeperiod = "2036_2065"



    pre_y_file1 = readfile("/pre_yearly_{:}_ysum_timmean_fldpctl.nc", timeperiod, "pre")
    pre_w_file1 = readfile("/pre_winter_{:}_ysum_timmean_fldpctl.nc", timeperiod, "pre")
    pre_s_file1 = readfile("/pre_summer_{:}_ysum_timmean_fldpctl.nc", timeperiod, "pre")
    rech_file1 = readfile("/recharge_adjust_{:}_ysum_timmean_fldpctl.nc", timeperiod, "recharge")
    tmax25_file1 = readfile("/tmax25_{:}_ysum_timmean_fldpctl.nc", timeperiod, "tmax")
    tmax30_file1 = readfile("/tmax30_{:}_ysum_timmean_fldpctl.nc", timeperiod, "tmax")
...
data.to_csv(outpath + "AllMETfiles_DE_2036_2065_rcp26_85_indicators.csv")
data.to_csv("/data/hydmet/WIS_D/wis_d_indicators/model_selection/AllMETfiles_DE_2036_2065_rcp26_85_indicators.csv")
