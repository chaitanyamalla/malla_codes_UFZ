...
if __name__ == '__main__':
    ...
    for i in range(3):
        ...
        filename="pre_ug_yearly_{:}_ysum_timmean_fldpctl_difference.nc".format(timeperiod)
        data_precip=read_data(VAR="pre",filename=filename)


        filename="tmax_ug_gtc25_{:}_ysum_timmean_fldpctl_difference.nc".format(timeperiod)
        data_warmdays=read_data(VAR="tmax",filename=filename)
        ...
        sns.scatterplot(data_warmdays, data_precip, hue= rcps, palette= ["C2", "C0", "k"])
        #plt.scatter(data_summerdays,data_winterprecip)
        plt.xlabel("Number of days > 25°C, difference with 1971-2000")
        plt.ylabel("Yearly precipitation difference with historic 1971-2000(mm)")
        ...
        plt.savefig(save_results_to + timeperiod + '_yearlypre_tmax25_diff' + '.png')
        plt.clf()
