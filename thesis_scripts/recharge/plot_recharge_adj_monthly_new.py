...
# --------------------------------------------------
#  File:        plot_recharge_adj_monthly_new.py 
#
...
def process(filename, var):
    ...
    f=filename
    f=f.where(f>0)
    da1= f[ind].mean(dim=["lat","lon"]).to_dataframe(k)
    
    return(da1)

d0= read("ensmedian_monthly_"+ind+"_adjust_historic.nc") 
d1= read("ensmedian_monthly_"+ind+"_adjust_1_5K.nc")
d2= read("ensmedian_monthly_"+ind+"_adjust_2K.nc")
d3= read("ensmedian_monthly_"+ind+"_adjust_3K.nc")
...
# ax.set_xlabel("Month")
ax.set_ylabel("Recharge [mm/month]")
ax.set_title("Recharge (absolute values)")
ax.legend()
...
d = ax1.bar(x + 0.2, data3_abs["3K"], label="3 °C" ,width=w, color = "#10732E")
c = ax1.bar(x + w/2, data2_abs["2K"], label="2 °C" ,width=w,color = "#5AC219")
b = ax1.bar(x - w/2, data1_abs["1_5K"], label="1.5 °C" ,width=w,color = "#CBD83F")

ax1.set_xticks(x)
ax1.set_xticklabels(labels)
# ax1.set_xlabel("Month")
ax1.set_ylabel("Recharge [mm/month]")
ax1.set_title("Recharge (absolute change)")
ax1.legend()


########--------------------rel change bar plot#--------------------------

d = ax2.bar(x + 0.2, data3_rel["3K"], label="3 °C" ,width=w, color = "#40AB88")#107E22 #208731
c = ax2.bar(x + w/2, data2_rel["2K"], label="2 °C" ,width=w,color = "#2FD880")
b = ax2.bar(x - w/2, data1_rel["1_5K"], label="1.5 °C" ,width=w,color = "#CAC539")#D8DE88

ax2.set_xticks(x)
ax2.set_xticklabels(labels)
ax2.set_xlabel("Month")
ax2.set_ylabel("Recharge [%]")
ax2.set_title("Recharge (relative change)")
ax2.legend()
