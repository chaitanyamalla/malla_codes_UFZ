...
if __name__ == '__main__':
    ...
    for i in range(3):
        ...
        filename="recharge_{:}_ysum_timmean_fldpctl.nc".format(warmperiod)
        ...
        sns.scatterplot(data_recharge, data_yearlyprecip, hue= gcms, s=20)
        plt.title(warmperiod)
        plt.xlabel("Average Yearly recharge [mm/yr]")
        plt.ylabel("Average Yearly precipitation [mm/yr]")
        plt.xlim(30, 80)
        plt.ylim(650, 950)
        #plt.show()

        plt.savefig(save_results_to + warmperiod + '_yearlyprecip_vs_recharge_GCMs' + '.png')
        plt.clf()
