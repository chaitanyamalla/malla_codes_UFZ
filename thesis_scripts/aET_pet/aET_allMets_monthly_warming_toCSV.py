...
# --------------------------------------------------
#  File:        aET_allMets_monthly_warming_toCSV.py
#
...
def readfile(filename,var):
    ...
    for met in mets:
        ...
        gp1= data1.aET.groupby("time.month")
...
a_hist= readfile("aET_historic.nc",1)
a_1_5K = readfile("aET_1_5K.nc",2)
a_2K = readfile("aET_2K.nc",3)
a_3K = readfile("aET_3K.nc",4)

result= a_hist.join([a_1_5K,a_2K,a_3K])
result.to_csv(outpath+"aET_monthly_ensmedian.csv")
print(result)
