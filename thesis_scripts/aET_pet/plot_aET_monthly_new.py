...
def read(filename):
    d= xr.open_dataset(inpath+indi+"/"+filename)
    return d
...
########--------------------abs change bar plot#--------------------------
d = ax1.bar(x + 0.2, data3_abs["3K"], label="3 °C" ,width=w, color = "#C34E20")
c = ax1.bar(x + w/2, data2_abs["2K"], label="2 °C" ,width=w,color = "#D59044")
b = ax1.bar(x - w/2, data1_abs["1_5K"], label="1.5 °C" ,width=w,color = "#DFBD42")
ax1.set_xticks(x)
...
d = ax2.bar(x + 0.2, data3_rel["3K"], label="3 °C" ,width=w, color = "#C56824")
c = ax2.bar(x + w/2, data2_rel["2K"], label="2 °C" ,width=w, color = "#A09F57")
b = ax2.bar(x - w/2, data1_rel["1_5K"], label="1.5 °C" ,width=w,color = "#EADEB8")
...
# plt.savefig(outpath+indi+"/"+ind+"_all3_monthly.png")
# plt.show()
res_abs= data1_abs.join([data2_abs,data3_abs])
res_rel= data1_rel.join([data2_rel,data3_rel])

res_abs.to_csv(outpath+indi+"/"+ind+"_abs_change_monthly.csv")
res_rel.to_csv(outpath+indi+"/"+ind+"_rel_change_monthly.csv")
